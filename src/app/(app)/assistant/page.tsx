  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <h1 className="text-2xl font-semibold">Ask about people</h1>
        <p className="text-sm text-[var(--muted-foreground)]">
          Lookup uses the live organisation record. No language model is connected.
        </p>
      </div>
      ...
      <Card className="border-[#c9a227]/40 bg-[#fbf6e8]">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-medium">Disabled until privacy policies are cross-checked</p>
            <p className="mt-1 text-sm text-[var(--muted-foreground)]">
              Generated commentary, inferred biographies, and any send of employee data to a model stay off. Admins can
              mark the policy review on Administration. That still does not hook up an AI.
            </p>
            <Badge tone="gold" className="mt-2">
              {locked ? 'Privacy review outstanding' : 'Review marked complete · model not connected'}
            </Badge>
          </div>
        </div>
      </Card>
      ...
      <Card className="space-y-3">
        <p className="text-sm">Search the directory. Facts come only from stored title, manager, and group fields.</p>
        <div className="flex flex-wrap gap-2">
          <Input
            className="min-w-[16rem] flex-1"
            placeholder="Name or title, e.g. Amelia or Head of Data"
            value={q}
          ...
          <Button variant="outline" disabled title="Blocked until privacy review is complete and a model is connected">
            Ask assistant
          </Button>
        </div>
        {lookup.data ? (
